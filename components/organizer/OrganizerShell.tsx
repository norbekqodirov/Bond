...
import { organizerNav } from "@/lib/organizer-navigation";
...
export function OrganizerShell({ user, permissions, children }: OrganizerShellProps) {
  const [collapsed, setCollapsed] = useState(false);
  const pathname = usePathname();
  const t = useTranslations("organizer");
  const locale = useLocale();

  const visibleNav = organizerNav.filter((item) => permissions.includes(item.permission));
  ...
  return (
    <div className="flex min-h-screen bg-slate-50">
      ...
      >
        ...
        <div className="px-4 pb-4 text-xs text-muted-foreground">v1.0</div>
      </aside>
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem
                  onClick={async () => {
                    ...
                    window.location.href = `/${locale}/organizer/login`;
                  }}
                >
